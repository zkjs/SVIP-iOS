...
class PhoneFirstVC: UIViewController {
  ...
  override func loadView() {
    NSBundle.mainBundle().loadNibNamed("PhoneFirstVC", owner:self, options:nil)
  }
  ...
  override func viewDidLoad() {
    ...
    if AccountManager.sharedInstance().phone.isEmpty == true {
  ...
  @IBAction func tappedOKButton(sender: UIButton) {
    ...
    ZKJSHTTPSMSSessionManager.sharedInstance().verifySmsCode(code, mobilePhoneNumber: phone) { (success: Bool, error: NSError!) -> Void in
      if success {
        self.navigationController?.pushViewController(PhoneSecondVC(), animated: true)
      } else {
...
extension PhoneFirstVC: UITextFieldDelegate {
